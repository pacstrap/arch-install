...
read -p "pvcreate --dataalignment 1m /dev/sda3"
pvcreate --dataalignment 1m /dev/sda3
continue

read -p "vgcreate vg1 /dev/sda3"
vgcreate vg1 /dev/sda3
continue

read -p "lvcreate -L 128GB vg1 -n lv_root"
lvcreate -L 128GB vg1 -n lv_root
continue

read -p "lvcreate -L 128GB vg1 -n lv_home"
lvcreate -L 128GB vg1 -n lv_home
continue
...
read -p "mkfs.fat -F32 -n BOOT /dev/sda1"
mkfs.fat -F32 -n BOOT /dev/sda1
continue

read -p "mkswap -L SWAP /dev/sda2"
mkswap -L SWAP /dev/sda2
continue

read -p "swapon /dev/sda2"
swapon /dev/sda2
continue
...
read -p "mount /dev/sda1 /mnt/boot && mount /dev/vg1/lv_home /mnt/home && mount /dev/vg1/lv_storage /mnt/storage"
...
read -p "pacstrap -i /mnt base base-devel linux linux-firmware linux-headers amd-ucode nano reflector lvm2 networkmanager"
pacstrap -i /mnt base base-devel linux linux-firmware linux-headers amd-ucode nano reflector lvm2 networkmanager
continue
