...
printf "\e[1;32m$ mkfs.vfat -n BOOT /dev/nvme0n1p1\e[0m\n"; confirm;
...
printf "\e[1;32m$ pacstrap -i /mnt base base-devel linux linux-firmware linux-headers intel-ucode nano git reflector rsync lvm2 networkmanager\e[0m\n"; confirm;
pacstrap -i /mnt base base-devel linux linux-firmware linux-headers intel-ucode nano git reflector rsync lvm2 networkmanager
...
cp gnome.sh /mnt
arch-chroot /mnt /bin/bash -c "chmod +x gnome.sh && ./gnome.sh"

rm /mnt/gnome.sh

printf "\e[1;32mDone. Now you can exit, umount -R /mnt, reboot...\e[0m\n"
exit
