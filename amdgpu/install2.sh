...
#read -p "useradd -m -g users -G wheel -s /bin/bash skippy"
#useradd -m -g users -G wheel -s /bin/bash skippy
#passwd skippy
#continue
...
read -p "nano /etc/mkinitcpio.conf (...block lvm2 filesystems resume... && MODULES=(amdgpu radeon))"
...
# pacman -S gnome dconf-editor ghex gnome-{sound-recorder,todo,tweaks,usage} celluloid lollypop terminator python-nautilus 
# continue
...
#read -p "systemctl disable dhcpcd.service"
#systemctl disable dhcpcd.service
#continue

#read -p "systemctl disable netctl.service"
